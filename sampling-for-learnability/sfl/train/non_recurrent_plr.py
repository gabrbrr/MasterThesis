...
def sample_trajectories(
    ...
    def sample_step(carry, _):
        ...
        # --- REMOVED --- (Squeezing likely not needed if network outputs (N,) directly)
...
def evaluate(
    ...
    """This runs the agent on the environment, given an initial state and observation, and returns (states, rewards, episode_lengths)
...
def update_actor_critic(
    ...
    Args:
        ...
        # --- REMOVED ---
        # init_hstate (chex.ArrayTree): 
        batch (chex.ArrayTree): obs, actions, log_probs, values, targets, advantages
        num_envs (int): 
        n_steps (int): 
        n_minibatch (int): 
        n_epochs (int): 
    ...
    # +++ ADDED +++: Flatten the batch for non-recurrent update
...
class ActorCritic(nn.Module):
    """This is an actor critic class (non-recurrent)
    """
    ...
    # --- REMOVED --- (No longer need initialize_carry)
    # @staticmethod
...
def main(config):
    ...
    def create_train_state(rng) -> TrainState:
        ...
        # --- MODIFIED ---: Create a batch of (N_ENVS, ...) not (T, N_ENVS, ...)
        obs = jax.tree_map(
        ...
        # --- MODIFIED ---
        network = ActorCritic(env.action_space(env_params).n, is_recurrent=False)
        # --- MODIFIED ---
        network_params = network.init(rng, obs) # No init_x, no initialize_carry
    ...
    def train_step(carry: Tuple[chex.PRNGKey, TrainState], _):
        ...
        def on_new_levels(rng: chex.PRNGKey, train_state: TrainState):
            ...
            # --- MODIFIED ---
            (rng, train_state), losses = update_actor_critic( # --- RENAMED ---
                rng,
                train_state,
                # --- REMOVED ---
                # ActorCritic.initialize_carry((config["NUM_ENVS"],)),
