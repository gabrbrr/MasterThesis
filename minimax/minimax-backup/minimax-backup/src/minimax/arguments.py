...
# ---- LTLEnv args for student model ----
student_ltl_model_parser = parser.add_subparser(
    name='student_ltl_model',
    prefix='student',
    dest="student_model",
    dependency={'env_name': 'LTLEnv*'})
student_ltl_model_parser.add_argument(
    '--text_embedding_size',
    type=int,
    default=32,
    nargs='?',
    choices=['gru', 'lstm', 's5'],
    help='text_embedding_size')
student_ltl_model_parser.add_argument(
    '--output_dim',
    type=int,
    default=4,
    help='number of output actions')
...
# ======== Envoronment-specific subparsers ======== 
# -------- LTLEnv --------
env_ltl_parser = parser.add_subparser(
    name='ltl',
    prefix='ltl',
    dependency={'env_name': ['LTLEnv']},
    dest='env')
env_ltl_parser.add_argument(
    '--grid_size',
    type=int,
    default=5,
...
env_ltl_parser.add_argument(
    ...
    help='list of letters in the map')
env_ltl_parser.add_argument(
    '--use_fixed_map',
    type=str2bool, 
    nargs='?', 
    ...
    help='Use fixed map')
env_ltl_parser.add_argument(
    '--use_agent_centric_view',
    type=str2bool, 
    nargs='?', 
    const=True, 
    default=False,
    help='use_agent_centric_view')
env_ltl_parser.add_argument(
    '--timeout',
    type=int, 
    nargs='?', 
...
env_ltl_parser.add_argument(
    ...
    nargs='?', 
    const=True, 
    default=len(set("abcdefghijkl")),
    help='num_unique_letters')



# -------- LTLEnv UED --------
ltl_ued_parser = parser.add_subparser(
    name='ltl_ued',
    prefix='ltl_ued',
    dependency={'env_name': ['LTLEnv'], 'train_runner': 'paired'},
    dest='ued_env')
ltl_ued_parser.add_argument(
    '--replace_wall_pos',
    type=str2bool, 
    nargs='?', 
    const=True, 
...
# -------- Maze eval arguments. --------
ltl_eval_parser = parser.add_subparser(
    name='ltl_eval',
    prefix='ltl_test',
    dependency={'env_name': 'LTLEnv*'},
    dest='eval_env'
)
# ltl_eval_parser.add_argument(
#     "--see_agent", 
#     type=str2bool, 
#     nargs='?', 
#     const=True, 
