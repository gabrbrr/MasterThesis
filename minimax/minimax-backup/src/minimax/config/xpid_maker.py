...
def _get_algo_info_ppo(p, role):
	if role == 'student':
		lr = str(p.lr)
		if 'lr_final' in p:
			lr_final = '' if p.lr_final is None or p.lr_final == p.lr else str(p.lr_final)
			if len(lr_final) > 0:
				lr = f"{lr}_{lr_final}"
		...
		if 'teacher_lr_final' in p:
			teacher_lr_final = str(p.lr_final) if p.teacher_lr_final is None else str(p.teacher_lr_final)
		else:
