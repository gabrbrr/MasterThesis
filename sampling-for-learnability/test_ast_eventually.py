...
def visualize_ast(
    ...
    for i in range(n_node):
        ...
        if token_id in TOKEN_MAP:
            ...
            # It's a proposition
            prop_index = token_id - 7 # (PROP_OFFSET + 1)
...
def test_and_visualize():
    ...
    PROP_TO_INT = {prop: i + 1 for i, prop in enumerate(PROPS)}
    INT_TO_PROP = {i + 1: prop for i, prop in enumerate(PROPS)}
    ...
    print(ltl_tuple_to_string(decode_formula(state,INT_TO_PROP)))
    print("Sampled State (already_true):", state.already_true)
    graph = build_ast_from_matrix(state)
